...
//  3. 无锁（单线程）：不内置任何锁，由调用方保证并发安全；多 goroutine 并发
//     读写同一实例会产生数据竞争，需调用方自行加锁。
...
//
// 注意：无锁实现下，遍历期间并发修改跳表会导致数据竞争，需由调用方保证
// 遍历期间不发生并发写。
...
func (sl *SkipList[K, V]) Ceiling(key K) (K, V, bool) {
	...
	var n *node[K, V]
	if sl.desc {
		// 原生降序：candidate 为第一个 <= key（自然序），Ceiling 需取其前驱（最小 >= key）
		if candidate != nil && sl.cmp(candidate.key, key) == 0 {
			n = candidate
		} else {
			n = preds[0]
...
func (sl *SkipList[K, V]) Floor(key K) (K, V, bool) {
	...
	var n *node[K, V]
	if sl.desc {
		// 原生降序：candidate 即第一个 <= key（自然序），即最大 <= key
		if candidate == nil {
			return zeroK, zeroV, false
		}
		n = candidate
	} else {
		// 原生升序：命中返回 candidate，否则取前驱（最大 <= key）
		if candidate != nil && sl.cmp(candidate.key, key) == 0 {
			n = candidate
		} else {
			n = preds[0]
...
// Iter 是 SkipList 的迭代器，用于按序遍历元素。
//
// 迭代器在创建（NewAscIter / NewDescIter）时保存当前节点的遍历起点或快照。
// 无锁实现不持有任何锁，遍历期间请勿并发修改跳表（需由调用方保证同步）。
//
// 沿原生方向的迭代通过前向指针逐节点前进，额外内存 O(1)；逆原生方向的迭代
// 由于跳表仅维护前向指针，创建时先收集节点快照再倒序返回，额外内存 O(n)。
//
// 推荐用法（defer Close 释放迭代器内部资源）：
//
//	it := sl.NewAscIter()
//	defer it.Close()
//	for {
...
type Iter[K any, V any] struct {
	...
	closed  bool          // 是否已调用 Close
}

// NewAscIter 创建一个升序迭代器，按 key 升序遍历所有元素。
//
// 无锁实现不持有锁；遍历期间请勿并发修改跳表。原生降序时需快照（O(n) 内存）。
//
// 使用示例：
//
//	it := sl.NewAscIter()
...
//
// 无锁实现不持有锁；遍历期间请勿并发修改跳表。原生升序时需快照（O(n) 内存），
// 原生降序时零快照。
//
// 使用示例：
//
