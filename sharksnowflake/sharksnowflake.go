...
//  3. 不包含 worker / 机器 ID，这是既定设计，不会加入。
//     本算法只保证单机、同一 Snowflake 实例内唯一；多机防撞靠外部手段（如 Redis 预分配列表）。
...
//
// 序列打满 / 时钟回拨：没有 worker id 可挪号段，只能等到下一秒，持锁等待是正确行为。
//
...
func (s *Snowflake) Generate() int64 {
	...
	for {
		...
		if s.timeStamp == timestamp {
			// 同一秒内：序列号自增
			s.sequenceId++
			// 序列号达上限：没有 worker id，无法换号段，只能等下一秒（持锁等待是正确行为）
